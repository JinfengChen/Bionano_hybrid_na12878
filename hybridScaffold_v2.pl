...
my $BestRef=$config{'align1.BestRef'};
...
@$cmdRef = ($refaligner, "-f", "-ref", $ngs_cmap, "-i", $opts{b}, "-o", "align1", "-T", $T, "-A", $A, "-biaswt", $biaswt, "-resSD", $resSD, "-FN", $FN, "-FP", $FP, "-res", $res, "-sf", $sf, "-sd", $sd, "-M", $M, "-extend", $extend, "-BestRef", $BestRef, "-outlier", $outlier, "-endoutlier", $endoutlier, "-deltaX", $deltaX, "-deltaY", $deltaY, "-xmapchim", $xmapchim, "-mres", $mres, "-RepeatMask", split(/\s+/,$RepeatMask), "-RepeatRec", split(/\s+/, $RepeatRec),  "-maxmem", $maxmem, "-maxthreads", $maxthreads, "-stdout", "-stderr", "-hashgen", split(/\s+/, "5 3 2.4 1.5 0.05 5.0 1 1 1"), "-hash", "-hashdelta", 50);
...
#run MergeNGS_BN.R
#***** however, run against merged v1 hybrid and v1 bioanano Naive
my $mergedHybridBnNaive = $opts{b};
$mergedHybridBnNaive =~ s/step2\.hybrid\.cmap$/v1.hybrid.BN.naive.cmap/;
die "ERROR: The merged Hybrid Bn Naive V1 file $mergedHybridBnNaive does not exist\n" if (! -e $mergedHybridBnNaive);
...
$RepeatMask=$config{'mergeNGS_BN.RepeatMask'};
$RepeatRec=$config{'mergeNGS_BN.RepeatRec'};
...
#	@$cmdRef = ("Rscript", "./scripts/MergeNGS_BN.R", "$opts{o}/mergeNGS_BN", $refaligner, $merge_Tvalue, "./", "mergeNGS_BN.log", "../assignAlignType/assignAlignType_r.cmap", $opts{b}, $id_shift, $max_merge_rounds, $maxthreads, $endoutlier, $outlier, $biaswt, $sd, $res, $sf, split(/\s+/, $RepeatMask), split(/\s+/, $RepeatRec));
	@$cmdRef = ("$^X", "./scripts/MergeNGS_BN.pl", "$opts{o}/mergeNGS_BN", $refaligner, $merge_Tvalue, "./", "mergeNGS_BN_script.log", "../assignAlignType/assignAlignType_r.cmap", $mergedHybridBnNaive, $id_shift, $max_merge_rounds, $maxthreads, $endoutlier, $outlier, $biaswt, $sd, $res, $sf, split(/\s+/, $RepeatMask), split(/\s+/, $RepeatRec)); #######################split(/\s+/, $pairmerge), $readparameters);
